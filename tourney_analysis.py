...
sts = st.getSeasonalStats(ts, strat='mest')
...
scale = RobustScaler()
all_features = Pipeline([('s1', FeatureUnion([('kpca_lin', kpca_lin),
                                              ('kpca_rbf', kpca_rbf),
...
nfeats = pd.DataFrame(all_features.fit_transform(features, winner))
...
rf_final = GridSearchCV(rforest,
                        ...
                        cv=cv)
rf_final.fit(rfe.transform(Xt), yt)

print('Log Losses')
#print('NN: {:.2f}'.format(log_loss(ys_ll, nn_final.predict_proba(Xs))))
print('RF: {:.2f}'.format(log_loss(ys_ll, rf_final.predict_proba(rfe.transform(Xs)))))

view = ttu.loc[ttu['Season'] > split_yr, ['GameID', 'Season', 'TID', 'OID']]
view['T%'] = (np.round(rf_final.predict_proba(rfe.transform(Xs))[:, 1], 2) * 100).astype(np.int)
view['O%'] = (np.round(rf_final.predict_proba(rfe.transform(Xs))[:, 0], 2) * 100).astype(np.int)
